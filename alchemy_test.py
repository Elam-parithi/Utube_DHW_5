...
def playlists_write(playlist_datum, chid):
    ...
    if session.query(Playlists).filter_by(playlist_ID=match_it).first():
        # _rollback(f"Playlists with ID {match_it} already exists. Skipping insertion.")
        return
    try:
        play = Playlists(
        ...
        session.add(play)
        session.commit()
        # print(f"Playlist {playlist_datum['playlist_title']} inserted successfully.")
    except IntegrityError:
...
print('filepath_list: ==>', filepath, type(filepath), len(filepath))
...
for files in filepath:
    ...
    print(f"channel_chid:{channel_chid}")
    for playlist_data in channel_data["playlist"]:
        ...
        for videos_dict in playlist_data["videos"]:
            ...
            for comment_dict in video_data["Comments"].values():
                ...
                # print(comment_dict[next(iter(comment_dict))])
